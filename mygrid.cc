...
FILE *pfile1_, *pfile2_, *pfile3_, *pfile4_, *pfile5_, *pfile6_, *pfile7_, *pfile8_, *pfile9_;
int counter = 0;
float residual = 0;
float accumolator = 0;
int alive = 0;
...
MyGridAgent::MyGridAgent() : Agent(PT_MYGRID)
{
        ...
        posx=0.0;
        posy=0.0;
        min_e = 0.02;       //minimum energy for communicating
	total_hopcount = 0; 
//        srand(clock());
//        bind("X_", &posx);	
//        bind("Y_", &posy);	
...
void MyGridAgent::result(int non, float nop, int tot_event_nodes, int num_row)	//(non)= number of nodes (nop)= number of packets produced by a source
{
//        iNode = (MobileNode *) (Node::get_node_by_address(source_id));
//        iEnergy = iNode->energy_model()->energy();
	++row_count;
	...
	if (r_c == 1) {
		pfile8_ = fopen("out_load.txt","a");   
		fprintf (pfile8_, "\n");
		fprintf (pfile8_, "%d", r_c);
		fclose(pfile8_);
		pfile9_ = fopen("out_consumed.txt","a");   
		fprintf (pfile9_, "\n");
		fprintf (pfile9_, "%d", r_c);
		fclose(pfile9_);
		++r_c;
	}
	pfile8_ = fopen("out_load.txt","a");  //file for report the nodes' load 
	fprintf (pfile8_, "-%.4f", 1/energy);
	fclose(pfile8_);

	pfile9_ = fopen("out_consumed.txt","a");  //file for report the nodes' load 
	fprintf (pfile9_, "-%.4f", init_energy - energy);
	fclose(pfile9_);
...
//                        float energy = iEnergy;
		...
		if (strcmp(argv[1], "send") == 0) {
			...
			int number_row = atoi(argv[5]);												
                        result(nodes,nop,tot_event_nodes,number_row);						
			return (TCL_OK);
		}
	}
