...
set val(initialenergy) 200			    ;# value
# ===========================================================================
...
set n $opt(nn)
set ST 8100             ;#simulation time
#set ST 25020             ;#simulation time
...
#---------------start multi-hop communication_src (source) sends data packet for sinks (destination)---------
...
set R_event 75
while {$firetime < $ST} {
	set rnd_f1 [expr {1 + floor(rand()* $n_sensor)}]         ;#Random variable as source between 0 to 24 
	set event1 [expr int($rnd_f1)]				;#define a node as event point
	set event_nodes 0
	set event_nodes1 0
	set event_nodes2 0
	set event_nodes3 0
	set rnd_f2 [expr {1 + floor(rand()* $n_sensor)}]         ;#Random variable as source between 0 to 24 	
	set event2 [expr int($rnd_f2)]				;#define a node as event point	
	set xx [expr $postable(0,$event1)-$postable(0,$event2)]
	set yy [expr $postable(1,$event1)-$postable(1,$event2)]
	set xx [expr abs($xx)]
	set yy [expr abs($yy)]
	while {$xx <= $R_event && $yy <= $R_event} {
		set rnd_f2 [expr {1 + floor(rand()* $n_sensor)}]         ;#Random variable as source between 0 to 24 	
		set event2 [expr int($rnd_f2)]				;#define a node as event point	
		set xx [expr $postable(0,$event1)-$postable(0,$event2)]
		set yy [expr $postable(1,$event1)-$postable(1,$event2)]
	...
	set rnd_f3 [expr {1 + floor(rand()* $n_sensor)}]         ;#Random variable as source between 0 to 24 	
	set event3 [expr int($rnd_f3)]				;#define a node as event point	
	set xx1 [expr $postable(0,$event1)-$postable(0,$event3)]
	set yy1 [expr $postable(1,$event1)-$postable(1,$event3)]
	set xx1 [expr abs($xx1)]
	set yy1 [expr abs($yy1)]
	set xx2 [expr $postable(0,$event2)-$postable(0,$event3)]
	set yy2 [expr $postable(1,$event2)-$postable(1,$event3)]
	set xx2 [expr abs($xx2)]
	set yy2 [expr abs($yy2)]
	while {($xx1 <= $R_event && $yy1 <= $R_event) || ($xx2 <= $R_event && $yy2 <= $R_event)} {
		set rnd_f3 [expr {1 + floor(rand()* $n_sensor)}]         ;#Random variable as source between 0 to 24 	
		set event3 [expr int($rnd_f3)]				;#define a node as event point	
		set xx1 [expr $postable(0,$event1)-$postable(0,$event3)]
		set yy1 [expr $postable(1,$event1)-$postable(1,$event3)]
	...
	for {set i 1} {$i < $sensor } {incr i} {	;#finding neighboring nodes at event point
		set xx1 [expr $postable(0,$event1)-$postable(0,$i)]
	        set yy1 [expr $postable(1,$event1)-$postable(1,$i)]
	        set pow_xx1 [expr $xx1*$xx1]
	        set pow_yy1 [expr $yy1*$yy1]
	        set sum_xy1 [expr $pow_xx1 + $pow_yy1]
	        set dton1 [expr sqrt($sum_xy1)]
		set xx2 [expr $postable(0,$event2)-$postable(0,$i)]
	        set yy2 [expr $postable(1,$event2)-$postable(1,$i)]
	        set pow_xx2 [expr $xx2*$xx2]
	        set pow_yy2 [expr $yy2*$yy2]
	        set sum_xy2 [expr $pow_xx2 + $pow_yy2]
	        set dton2 [expr sqrt($sum_xy2)]
		set xx3 [expr $postable(0,$event3)-$postable(0,$i)]
	        set yy3 [expr $postable(1,$event3)-$postable(1,$i)]
	        set pow_xx3 [expr $xx3*$xx3]
	        set pow_yy3 [expr $yy3*$yy3]
	        set sum_xy3 [expr $pow_xx3 + $pow_yy3]
	        set dton3 [expr sqrt($sum_xy3)]
		if {$dton1 <= $R_event} {
			set src($event_nodes) $i
			set event_nodes [expr $event_nodes + 1]
			set src1($event_nodes1) $i
			set event_nodes1 [expr $event_nodes1 + 1]
		}
		if {$dton2 <= $R_event} {
			set src($event_nodes) $i
			set event_nodes [expr $event_nodes + 1]
			set src2($event_nodes2) $i
			set event_nodes2 [expr $event_nodes2 + 1]
		}
		if {$dton3 <= $R_event} {
			set src($event_nodes) $i
			set event_nodes [expr $event_nodes + 1]
			set src3($event_nodes3) $i
			set event_nodes3 [expr $event_nodes3 + 1]
		}
	}

	set total_event_nodes [expr $total_event_nodes + $event_nodes]

	set event_x1 $postable(0,$event1)
	set event_y1 $postable(1,$event1)

	set event_x2 $postable(0,$event2)
	set event_y2 $postable(1,$event2)

	set event_x3 $postable(0,$event3)
	set event_y3 $postable(1,$event3)
...
#		} 
		for {set j 0} {$j < $event_nodes1} {incr j} {		;#sending the first packets to the static sink
			set xpos $postable(0,$src1($j))
			set ypos $postable(1,$src1($j))
			$ns_ at $multifire "$p($src1($j)) send DATA $xpos $ypos"
			set multifire [ expr $multifire + 0.020 ]
		}  
		set multifire [ expr $multifire + 0.050 ]    
		for {set j 0} {$j < $event_nodes2} {incr j} {		;#sending the first packets to the static sink
			set xpos $postable(0,$src2($j))
			set ypos $postable(1,$src2($j))
			$ns_ at $multifire "$p($src2($j)) send DATA $xpos $ypos"
			set multifire [ expr $multifire + 0.020 ]
		} 
		set multifire [ expr $multifire + 0.050 ]     
		for {set j 0} {$j < $event_nodes3} {incr j} {		;#sending the first packets to the static sink
			set xpos $postable(0,$src3($j))
			set ypos $postable(1,$src3($j))
			$ns_ at $multifire "$p($src3($j)) send DATA $xpos $ypos"
			set multifire [ expr $multifire + 0.020 ]
		}      
...
#-------------- Periphery Sink movement in TCL-------------------
...
set dimx $fullx
set ix [expr $ix-1]
for {set dimy 20} {$dimy <= $fully} {set dimy [expr $dimy + $R]} {
	set s1(0,$ix) $dimx
	set s1(1,$ix) [expr $dimy + $R]
	set ix [expr $ix+1]
}
set dimy $fully
set ix [expr $ix-1]
...
set dimx 20
set ix [expr $ix-1]
for {set dimy $fully} {$dimy >= 20} {set dimy [expr $dimy - $R]} {
	set s1(0,$ix) $dimx
	set s1(1,$ix) [expr $dimy - $R]
	set ix [expr $ix+1]
}
...
#		$ns_ at $energy_update_interval "$p($e) send EUPDATE"     ;# send ENERGY UPDATE packets
		...
		set packet_send_period [ expr $packet_send_period + 0.003 ]
	}
	set energy_update_interval [ expr $energy_update_interval + 60 ]
}

#---------------getting results at the end of program-----------

proc result {} {
	global ns_ p sensor ST nop total_event_nodes num_row
	set nodes [expr $sensor-1]
	for {set e 1} {$e < $sensor} {incr e} {
#		$ns_ at $ST "$p($e) results $nodes $period"     ;# get results like residual energy
		$ns_ at $ST "$p($e) results $nodes $nop $total_event_nodes $num_row"     ;# get results like residual energy
	}
}
